...
export const AssetHistoryModal: React.FC<AssetHistoryModalProps> = ({
  ...
  const targetId = asset?.id || goal?.linkedAssetId;
  const goalId = goal?.id;
  const itemName = asset?.name || goal?.name || 'Tài sản';
  const itemType = asset?.type || goal?.assetType || 'stock';
  const unit = goal?.unit || asset?.unit || (itemType === 'stock' ? 'CP' : itemType === 'gold' ? 'chỉ' : 'VNĐ');
  ...
  const currentItemTxs = allTxs.filter((t) => {
    ...
    if (asset?.name && t.assetName && t.assetName.toLowerCase() === asset.name.toLowerCase()) return true;
    return false;
  });
  ...
  const handleQtyChange = (val: string) => {
    ...
    if (qty > 0 && price > 0) {
      setTxTotalAmountStr(formatNumberString(Math.round(qty * price)));
    }
  };
  ...
  const handleSaveTransaction = (e: React.FormEvent) => {
    ...
    const newMarketVal = newTotalQty > 0 ? (currentMarketPricePerUnit > 0 ? newTotalQty * currentMarketPricePerUnit : newTotalCost) : newTotalCost;
    ...
    if (asset) {
      updatedAsset = {
        ...asset,
        quantity: newTotalQty > 0 ? newTotalQty : asset.quantity,
        costPrice: newTotalCost,
        unitPrice: newAvgCost,
        currentPrice: currentMarketPricePerUnit > 0 ? currentMarketPricePerUnit : asset.currentPrice,
        amount: newMarketVal > 0 ? newMarketVal : asset.amount,
    ...
    if (goal) {
      updatedGoal = {
        ...goal,
        totalBought: newTotalQty > 0 ? newTotalQty : goal.totalBought,
        costPrice: newTotalCost,
        unitPrice: newAvgCost,
        currentPrice: currentMarketPricePerUnit > 0 ? currentMarketPricePerUnit : goal.currentPrice,
      };
    }
  ...
  const handleDeleteTx = (txId: string) => {
    if (!confirm('Bạn có chắc chắn muốn xóa đợt giao dịch này?')) return;
            ...
            {isEditingMarketPrice ? (
              <div className="flex items-center gap-1 mt-1">
                <input
                  ...
                  onChange={(e) => setCustomMarketPriceStr(formatNumberString(e.target.value))}
                  placeholder="Giá mới..."
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 text-xs">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => setTxTotalAmountStr(formatNumberString(e.target.value))}
                    placeholder="0"
