...
import { Layers, Scale, Target } from 'lucide-react';
...
export const FixedBottomNav: React.FC<FixedBottomNavProps> = ({
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/90 shadow-[0_-4px_16px_rgba(0,0,0,0.06)] px-2 sm:px-4 py-1.5 safe-area-pb">
      <div className="max-w-xl mx-auto grid grid-cols-3 gap-1 sm:gap-2">
        {/* Tab 1: Tháp Tài Sản */}
        <button
          type="button"
          onClick={() => onSwitchTab('pyramid')}
          className={`flex flex-col items-center justify-center py-1.5 px-1 sm:px-3 rounded-xl transition cursor-pointer relative ${
            currentTab === 'pyramid'
        ...
        >
          ...
          <span className="text-[10px] sm:text-xs mt-0.5 tracking-tight truncate">Kế Hoạch Mục Tiêu</span>
        </button>
      </div>
    </nav>
  );
